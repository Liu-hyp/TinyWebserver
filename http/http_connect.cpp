...
{
    connectionRAII mysqlcon(&this->mysql,connPool);
}
...
//TODO::根据服务器处理HTTP请求的结果，决定返回给客户端的内容，即生成响应报文
...
void http_conn::process(http_conn* user, connection_pool* connPool)
{
    //数据库连接池取连接
    user->pick_one_sql_connect(connPool);
